...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('strong[class^="readMore"]').forEach((button) => {
        button.addEventListener('click', () => {
            const projectDiv = button.closest('div');
            if (!projectDiv) {
                console.error('No project container found for:', button);
                return; 
            }

            const descContainer = projectDiv.querySelector('.desc');
            if (!descContainer) {
                console.error('No .desc container found within:', projectDiv);
                return;
            }

            const existingParagraph = descContainer.querySelector('p.additional-content');

            if (!existingParagraph) {
                const paragraph = document.createElement('p');
                paragraph.classList.add('additional-content');
                paragraph.textContent = readMoreTexts[button.classList[0]];
